...
### Step W.6.6: Wire "Create spec" button in IssueColumn → SpecDialog → start_ticket_run
...
### Step W.7.1: `PermissionCard` component
...
### Step W.7.2: ActivityColumn renders PermissionCard inline for `perm` events
...
### Step W.8.2: Build `SettingsModal` shell with `GeneralTab` + `HistoryTab`
...
### Step W.8.3: Wire `SettingsModal` into `App.tsx` from the header settings icon
...
## Phase 9 — TUI palette + title bar

### Step T.9.1: New `theme` module with color constants
...
### Step T.9.2: Title bar (28 px, traffic lights + centered text)
...
- Integrate into `draw_app` above the existing two-pane layout — but only after the issue header / pipeline bar / tab bar are also shipped (T.10.x), so for now wire it as the topmost row of `draw_app` and shift everything else 1 row down.
...
## Phase 10 — TUI pipeline strip + tabs

### Step T.10.1: Issue header strip
...
### Step T.10.3: Pipeline footer hint
...
### Step T.10.5: Wire `1` / `2` / `3` keys to switch panes
...
## Phase 11 — TUI body panes restyle

### Step T.11.1: Logs pane — column-aligned rows
...
### Step T.11.2: Chat pane — message blocks
...
### Step T.11.3: Issue pane — id, title, labels, description, acceptance
...
## Phase 12 — TUI permission card + status line

### Step T.12.1: Inline permission card in logs
...
### Step T.12.2: Wire `y` / `s` / `n` keys to resolve permission
...
### Step T.12.3: Mode indicator in status line
...
### Step T.12.4: Flash messages on the status line
...
### Step T.12.5: Help overlay (`?` toggle)
...
### Step T.12.6: Insert mode (`i` in chat/logs pane) + triage `i` in issue pane
...
### Step T.12.7: Wire all TUI views into `draw_app`

**Goal**: `draw_app` now lays out, top-to-bottom: title bar (T.9.2) → issue header (T.10.1) → pipeline bar + hint (T.10.2/3) → tab bar (T.10.4) → active body pane (T.11.x) → status line (T.12.3). Help overlay renders above all.

**Depends on**: T.9.2, T.10.1, T.10.2, T.10.3, T.10.4, T.11.1, T.11.2, T.11.3, T.12.3, T.12.5.
...
### Step T.12.8: Delete dead TUI views (cockpit, old chat)
...
## Phase 13 — Tauri dense layout + chrome verification

### Step X.13.1: Wire `dense` prop through AppShell from Tauri detection
...
### Step X.13.2: Verify and lock `tauri.conf.json` window decorations
...
## Phase 14 — Cross-cutting cleanup + tech debt index

### Step X.14.1: Full-workspace test pass + screenshot refresh

**Goal**: Run the full test matrix and update any committed screenshots / fixtures that reflect the redesign.

**Depends on**: W.8.5, T.12.8, X.13.2.
...
### Step X.14.2: File tech-debt entries for deferred items
...
- [ ] W.8.4 Restyle DismissableBanner/DiffViewer
- [ ] W.8.5 Delete dead components
- [ ] CP-W
...
Phase 10 — TUI pipeline + tabs
- [ ] T.10.1 Issue header
- [ ] T.10.2 Pipeline bar boxes
- [ ] T.10.3 Pipeline hint
- [ ] T.10.4 Tab bar
- [ ] T.10.5 1/2/3 keys

Phase 11 — TUI body panes
- [ ] T.11.1 Logs pane
- [ ] T.11.2 Chat pane
- [ ] T.11.3 Issue pane

Phase 12 — TUI permission + status line
- [ ] T.12.1 Permission card
- [ ] T.12.2 y/s/n keys
- [ ] T.12.3 Status line + modes
- [ ] T.12.4 Flash lifecycle
- [ ] T.12.5 Help overlay
- [ ] T.12.6 Insert mode
- [ ] T.12.7 draw_app composition
- [ ] T.12.8 Delete dead views
- [ ] CP-T

Phase 13 — Tauri
- [ ] X.13.1 Wire dense
- [ ] X.13.2 Lock window decorations

Phase 14 — Cleanup + tech debt
- [ ] X.14.1 Full test pass + snapshots
- [ ] X.14.2 File tech-debt issues
...
3. **TUI `i` key conflict** — Confirmed: dual binding scoped by pane.
   Implemented in T.12.6 with explicit test cases for each scope.
