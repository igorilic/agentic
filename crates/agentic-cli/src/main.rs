...
use agentic_cli::args::{BackendKind, Cli, CliCommand};
...
async fn run_command(cli: Cli) -> Result<()> {
    ...
    match cli.command {
        CliCommand::Run {
            ...
            // Belt-and-suspenders: clap's `requires = "ticket"` on the `agents`
            // flag rejects `--scripted --agents foo` at parse time, but we also
            // guard at the application layer for a clearer error message.
            if !agents.is_empty() {
                anyhow::bail!(
        ...
        CliCommand::Doctor => cmd_doctor(),
        CliCommand::Migrate => cmd_migrate(&paths).await,
        CliCommand::Init {
            target,
            copilot,
            global,
...
async fn cmd_run(paths: &Paths, script_path: &std::path::Path) -> Result<()> {
    ...
    // Note: the orchestrator holds a bus clone (Sender) to pass to gate tasks,
    // so the broadcast channel never closes on drop(bus) alone. Give the
    // orchestrator 200 ms to process the final RunComplete event, then abort
    // it so the printer and persister see channel-close and exit naturally.
    drop(bus);
...
async fn cmd_run_ticket(
    ...
    // Shut down cleanly.
    // The orchestrator holds a bus clone so drop(bus) alone doesn't close the
    // channel. Give it 200 ms to process final RunComplete, then abort.
    drop(bus);
    tokio::time::sleep(std::time::Duration::from_millis(200)).await;
    orch_handle.abort();
    pers_handle.await.context("persister task")?;
